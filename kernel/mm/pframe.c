...
int pframe_lookup(struct mmobj *o, uint32_t pagenum, int forwrite,
		pframe_t **result) {
	KASSERT(NULL != o);
	KASSERT(NULL != result);
...
void pframe_migrate(pframe_t *pf, mmobj_t *dest) {
	...
	if (NULL != pframe_get_resident(dest, pf->pf_pagenum)) {
		...
		src->mmo_nrespages--;
		src->mmo_ops->put(src);
		list_insert_head(&pframe_hash[hash_page(dest, pf->pf_pagenum)],
				&pf->pf_hlink);
 ...
 */
static int pframe_fill(pframe_t *pf) {
	int ret;
	...
	*result = pframe_get_resident(o, pagenum);
	...
	if (*result != NULL) {
		...
		*result = pframe_alloc(o, pagenum);
		...
		if ((retVal = pframe_fill(*result)) < 0) {
			dbg(DBG_PRINT, "(GRADING3D 3)\n");
			pframe_free(*result);
			return retVal;
		}
...
void pframe_pin(pframe_t *pf) {
	...
	if (pf->pf_pincount == 0) {
		dbg(DBG_PRINT, "(GRADING3D 3)\n");
		list_remove(&pf->pf_link);
		/*	pf->pf_obj=NULL; *//*.. It's not mentioned in comments to free obj, hence commented. Not sure  */
		nallocated--;
	...
	list_iterate_begin(mmobj_bottom_vmas(pf->pf_obj), vma, vmarea_t, vma_olink)
				{
					...
					if ((pf->pf_pagenum >= vma->vma_off)
						...
						uintptr_t vaddr = (uintptr_t) PN_TO_ADDR(
								vma->vma_start + pf->pf_pagenum - vma->vma_off);
						/* And unmap it from that area's proc */
						if (NULL != vma->vma_vmmap->vmm_proc) {
							pt_unmap(vma->vma_vmmap->vmm_proc->p_pagedir,
									vaddr);
						}
					}
...
/* Handle phantom pframes*/
	...
	list_iterate_begin(&alloc_list, pf, pframe_t, pf_link)
			 ...
			 */
static __attribute__((unused)) void pageoutd_init(void) {
	/* initialize pageoutd_waitq: */
	sched_queue_init(&pageoutd_waitq);

	/* create and schedule pageoutd: */
	KASSERT(
			curproc && (PID_IDLE == curproc->p_pid) && "should be calling this from idleproc");
...
pageoutd_run(int arg1, void *arg2) {
	while (1) {
		...
		dbg(DBG_PFRAME, "PAGEOUT DEMAON: "
				"nfreepages_target=|%d| "
				"nfreepages_min=|%d| "
				"page_free_count=|%d|\n", nfreepages_target, nfreepages_min,
				page_free_count());
