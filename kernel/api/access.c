...
char **user_vecdup(argvec_t *uvec) {
	...
	if (NULL
			== (temp_kvec = (argstr_t *) kmalloc(
					(uvec->av_len + 1) * sizeof(argstr_t)))) {
		ret = -ENOMEM;
		goto fail;
	}
	if (NULL
			== (kvec = (char **) kmalloc((uvec->av_len + 1) * sizeof(char *)))) {
		ret = -ENOMEM;
		goto fail;
	}
	/* Copy over the array of argstrs */
	if (0
			> (ret = copy_from_user(temp_kvec, uvec->av_vec,
					(uvec->av_len + 1) * sizeof(argstr_t)))) {
		goto fail;
	}
...
int range_perm(struct proc *p, const void *avaddr, size_t len, int perm) {
	...
	for (start_addr = (uint32_t) avaddr; start_addr < end_addr; start_addr +=
			PAGE_SIZE) {
		dbg(DBG_PRINT, "(GRADING3D 3)\n");
