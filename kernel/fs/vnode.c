...
static int special_file_read(vnode_t *file, off_t offset, void *buf,
		size_t count);
static int special_file_write(vnode_t *file, off_t offset, const void *buf,
		size_t count);
...
static int vlookuppage(mmobj_t *o, uint32_t pagenum, int forwrite,
		pframe_t **pf);
...
extern void pframe_remove(void);
static mmobj_ops_t vnode_mmobj_ops = { .ref = vo_vref, .put = vo_vput,
		.lookuppage = vlookuppage, .fillpage = vreadpage, .dirtypage =
				vdirtypage, .cleanpage = vcleanpage };

/* vnode operations tables for special files: */
static vnode_ops_t bytedev_spec_vops = { .read = special_file_read, .write =
		special_file_write, .mmap = special_file_mmap, .create = NULL, .mknod =
NULL, .lookup = NULL, .link = NULL, .unlink = NULL, .mkdir = NULL,
		.rmdir = NULL, .readdir = NULL, .stat = special_file_stat, .fillpage =
				special_file_fillpage, .dirtypage = special_file_dirtypage,
		.cleanpage = special_file_cleanpage };

static vnode_ops_t blockdev_spec_vops = { .read = NULL, .write = NULL, .mmap =
NULL, .create = NULL, .mknod = NULL, .lookup = NULL, .link = NULL, .unlink =
NULL, .mkdir = NULL, .rmdir = NULL, .readdir = NULL, .stat = special_file_stat,
		.fillpage = NULL, .dirtypage = NULL, .cleanpage =
		NULL };

/*
 * Initialization:
 */
static __attribute__((unused)) void vnode_init(void) {
...
void vput(struct vnode *vn) {
	...
	dbg(DBG_VNREF, "vput: 0x%p, 0x%p ino %ld, down to %d, nrespages = %d\n", vn,
			vn->vn_fs, (long )vn->vn_vno, vn->vn_refcount - 1,
			vn->vn_nrespages);
...
int vfs_is_in_use(fs_t *fs) {
	...
	for (link = list->l_next; link != list; link = link->l_next) {
		...
		KASSERT(0 <= refs);
		KASSERT(
				((vn->vn_fs->fs_root == vn) && (1 <= refs))
						|| ((vn->vn_fs->fs_root != vn) && (0 <= refs)));

		if (((vn->vn_fs->fs_root == vn) && (1 < refs))
				|| ((vn->vn_fs->fs_root != vn) && refs)) {
			dbg(DBG_ALL,
					"Vnode %ld mode %x device %x flags %x is still in use with refcount=%d and %d res pages\n",
					(long )vn->vn_vno, vn->vn_mode, vn->vn_devid, vn->vn_flags,
					vn->vn_refcount, vn->vn_nrespages);
			ret = -EBUSY;
		}
	}
							...
							p, pframe_t, pf_olink)
								{
									if (pframe_is_dirty(p)) {
										if (0 > (err = pframe_clean(p))) {
											dbg(DBG_VFS,
													"vnode_flush_all: WARNING: failed to clean page %d of "
															"vnode %ld of fs %p of type %s\n",
													p->pf_pagenum,
													(long )v->vn_vno, v->vn_fs,
													v->vn_fs->fs_type);
										}
										KASSERT(
												(!err)
...
void vfs_flush_vnodes() {

	vnode_t *vn;
	pframe_t *p;
	list_iterate_begin(&vnode_inuse_list, vn, vnode_t, vn_link)
				{
					list_iterate_begin(&vn->vn_mmobj.mmo_respages,p, pframe_t, pf_olink)
								{
									if (pframe_is_dirty(p)) {
								...
								if (!S_ISBLK(vn->vn_mode)) {

									list_remove(&vn->vn_link);
									slab_obj_free(vnode_allocator,vn);
								}
							}list_iterate_end();
							/*Disregarding block devices*/
	vn->vn_fs->fs_op->umount = NULL;
	/*phantom pframes*/
	pframe_remove();
...
static int special_file_read(vnode_t *file, off_t offset, void *buf,
	...
	if (S_ISCHR(file->vn_mode)) {
		KASSERT(
				file->vn_cdev && file->vn_cdev->cd_ops
...
static int special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret) {
	...
	KASSERT(
			S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT, "(GRADING3A 5.a)\n");
...
static int vcleanpage(mmobj_t *o, pframe_t *pf) {
	...
	vnode_t *v = mmobj_to_vnode(o);
	return v->vn_ops->cleanpage(v, (int) PN_TO_ADDR(pf->pf_pagenum),
			pf->pf_addr);
}
